...
We prefer the analysis with throax length because without throax length the analysis seems to be wrong as can be seen in the QQ-Plot and Residual Scatter Plot shown in Section 3. Residuals seems to from a normal distribution, however, when we look at scatter plot, we can identify a pattern which indicates that the model without throax length is not representing the data well. QQ-Plot and the scatter plot of the model with throax length can be seen below. From this graph, we can see that residuals are from a normal distribution and the scatter plot seems to be spread randomly. Again, due to these reasons, we prefer the model with throax length.  
...
As seen in the graphs provided in Section 7, the assumption of normality does not seem to be violated, and the data appears reasonably homoskedastic - the variance of the residuals does not increase greatly for higher fitted values. 
