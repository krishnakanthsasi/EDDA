...
Each row represents a skill block. The students have been numbered in ascending order (1,2,3 in the first skill group, 4,5,6 in the second, etc.). Each column represents an interface assignment,  meaning student 3 will be assigned interface 1, student 1 to interface 2, and so on. 
...
pv <- searchaov$`Pr(>F)`[1]
```
The p-value for an ANOVA test is >`r cat(round( pv,3))`, which indicates that there is significant evidence to refute the null hypothesis. The search time does not seem to be the same for all interfaces.
...
plot(fitted(lmsearch),residuals(lmsearch))
```
The QQ-plot seems somewhat curved, and the scatterplot looks normal. 
...
The p-value for no treatment effect is `r cat(round(friedman.test(time,interface, skill)[[3]],3))`. The p-value is smaller than $\alpha$, so the null should be rejected. There is an effect of interface present. 
